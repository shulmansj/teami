...
The local registry is `.teami/domains.json`.
...
```text
.teami/domains/<domain_id>/linear.json
```
...
```js
{
  ...
  linear: {
    ...
    teamName: "Teami",
    webhookId: "webhook_...",
    cachePath: "C:/abs/repo/.teami/domains/support-ops/linear.json"
  },
  credentialTargets: {
...
Only fields with known values are emitted. `teami.domain_name` is not
