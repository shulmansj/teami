...
| `artifact_schema` | neutral envelope id `teami-run-artifact/v1` + `engine_version`/`function_version` |
...
- **Knob classification**: every behavioral knob is a Phoenix accepted asset OR factory-owned (see
  ...
  the `TEAMI_ALLOW_UNPINNED_RUNTIME` dev flag and emit `unpinned_runtime` + a Phoenix attribute
...
- **Optimization-evidence contract (every function, BY DECLARATION — checklist):** each function
  declares its `commit_effects` **and** its optional `outcome_observations` (the mirror of effects;
  ...
  `artifact_set_lineage`). Local Phoenix holds the full observable play-by-play (prompt, raw output, tool
  events), scrubbing **only** true secrets; the size cap is a safety net that folds overflow into a digest
  span + a root `evidence_unavailable` marker rather than dropping the trace. No function-specific tracing
  code — a synthetic second function inherits all of the above by DECLARATION alone (proven in
  `synthetic-second-function.test.mjs`). The outcome channel is placed but **unwired** (no senders).
