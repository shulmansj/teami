...
these surfaces, they do so through normal Agentic Factory development with explicit
...
Normal v1 setup must create or verify one private Agentic Factory behavior repo
...
adopter-owned resource even though Agentic Factory created it. A maintainer
...
- pushed refs must be inside `refs/heads/agentic-factory/promotion/*`.
...
| `tenant_id` | Agentic Factory tenant/workspace binding |
...
| Run retry artifacts | Ignored local run store such as `.agentic-factory/runs/` | Retry authority before Linear mutation; not a durable eval backend |
...
- Agentic Factory does not pretend raw evidence still exists.
...
- model provider credential rotation if Agentic Factory stored or mediated it.
...
anything leaves the machine, Agentic Factory shows a previewable manifest.
