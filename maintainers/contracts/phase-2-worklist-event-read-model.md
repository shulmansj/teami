...
| GitHub PR state | Yes, in GitHub | GitHub plus owner action; Agentic Factory may only create PRs or update PR bodies through the allowlisted client | Open/closed/merged state, current PR URL, branch/merge status when available | GitHub owns acceptance/decline surface. Local cache cannot override it. |
...
`.agentic-factory/promotion-candidates/`, but filename co-location is not
