...
export function verifyPollScopeApplied(pollResult, seededProjectId) {
  ...
  for (const domain of pollResult?.poll?.domains || []) {
    ...
    for (const entry of domain?.processed || []) {
      ...
      if (projectId !== expectedProjectId) offenders.push({ domainId, index, projectId });
      index += 1;
    }
  }
  return {
    ok: Boolean(expectedProjectId) && processedProjectIds.length > 0 && offenders.length === 0,
    processedProjectIds,
    offenders,
  };
}
...
async function stepProduce(ctx) {
  ...
  const content = buildProjectTemplateBody().replace("Linear project: {project_name}", `Linear project: ${name}`);
  ...
  const state = readLocalTriggerState(localTriggerStorePath(ctx.repoRoot));
  ...
  // committed issues; a `paused` run raised discovery/open questions. The loop
  ...
  return {
    ...
    detail: `run=${run.run_id} outcome=${run.status === "paused" ? "paused (discovery needed)" : run.status} trace=${ctx.traceId || "none"}`,
...
export async function stepTeardown(ctx, opts = {}) {
  ...
  try {
    ...
    return {
      ok: false,
      status: "fail",
      detail: `teardown failed while ${teardownAction || "cleaning up"} seeded project: ${e.message}`,
