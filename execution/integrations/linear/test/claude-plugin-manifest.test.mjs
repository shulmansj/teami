...
test("Claude plugin manifest declares Teami metadata and stdio MCP server", () => {
  ...
  assert.ok(
    server.args.includes("@shulmansj/teami@__TEAMI_VERSION__"),
    "manifest must keep the literal publish-time version token",
  );
  assert.equal(server.args.some((arg) => String(arg).includes("__TEAMI_VERSION__")), true);
