...
test("Claude plugin registration upgrades a stale installed version and verifies exact read-back", async () => {
  ...
  assert.equal(result.version, PACKAGED_PLUGIN_VERSION);
...
test("Claude plugin registration repairs a stale launcher even when its display version matches", async () => {
  const fakeClaude = createFakeClaudeRunner({
    ...
    installedVersion: PACKAGED_PLUGIN_VERSION,
...
function createFakeClaudeRunner({
  ...
  const state = {
    ...
    installedVersion: installedVersion || PACKAGED_PLUGIN_VERSION,
    installedPackageRef: installedPackageRef || installedVersion || PACKAGED_PLUGIN_VERSION,
    installFailed: false,
  };
  const calls = [];
  const runCommand = (command, args, options = {}) => {
    ...
    switch (args.join(" ")) {
      ...
      case "plugin install teami@teami --scope user":
        ...
        state.installedVersion = PACKAGED_PLUGIN_VERSION;
        state.installedPackageRef = PACKAGED_PLUGIN_VERSION;
